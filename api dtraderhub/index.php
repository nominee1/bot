...
function verify_options_rest_real_usd(string $token, string $appId = '71070'): array {
    ...
    if ($raw === false) {
        throw new RuntimeException('Could not reach Deriv Options API.');
    }

    $body = json_decode($raw, true);
    if (!is_array($body)) {
        throw new RuntimeException('Unexpected response from Deriv Options API.');
    }

    if (isset($body['errors']) && is_array($body['errors']) && count($body['errors']) > 0) {
        $msg = (string)($body['errors'][0]['message'] ?? 'Invalid token');
    ...
    if (isset($body['data']) && is_array($body['data'])) {
        $list = $body['data'];
    } elseif (isset($body['data']['accounts']) && is_array($body['data']['accounts'])) {
        $list = $body['data']['accounts'];
    ...
    foreach ($list as $row) {
        ...
        $type = strtolower((string)($row['account_type'] ?? $row['accountType'] ?? $row['type'] ?? ''));
        $isDemo = ($type === 'demo' || $type === 'virtual')
            || !empty($row['is_demo'])
            || !empty($row['is_virtual']);
        if ($isDemo) {
            continue;
        }
        $currency = strtoupper((string)($row['currency'] ?? $row['currency_code'] ?? 'USD'));
        if ($currency !== 'USD') {
            continue;
        }
        $loginid = (string)($row['account_id'] ?? $row['accountId'] ?? $row['id'] ?? $row['loginid'] ?? '');
        if ($loginid !== '') {
            $pick = ['loginid' => $loginid, 'currency' => $currency, 'is_virtual' => false];
            break;
        }
    }
    ...
    if ($pick === null) {
        fail('Token must be a REAL USD Options account, not demo.', 422);
    }
...
function verify_token_is_real(string $token, int $appId = 76100): array {
    ...
    try {
        ...
        $tryOptions = str_contains($msg, 'invalid')
            || str_contains($msg, 'token')
            || str_contains($msg, 'session')
            || str_contains($msg, 'unauthorized')
            || str_starts_with(trim($token), 'eyJ');

        if (!$tryOptions) {
            throw $e;
        }

        foreach (['71070', '66945', '76100'] as $oidApp) {
            try {
                return verify_options_rest_real_usd($token, $oidApp);
