...
export default function MultiStrategyBot() {
    ...
    const VOL_LIST = useRef<string[]>([
        'R_10', '1HZ10V', '1HZ15V', 'R_25', '1HZ25V', '1HZ30V', 'R_50', '1HZ50V', 'R_75', '1HZ75V', 'R_1
90V','R_100','1HZ100V',
    ...
    // Attach to the current socket's lifecycle; on open/close, bump epoch.
    useEffect(() => {
        ...
        // Re-run after each epoch so listeners move to the *new* connection
    }, [apiEpoch]);

    // Ensure API is fully ready before any BUY (covers readyState after account switch)
    ...
    const buy = async (ct: string, stake: number, mkt: string, dur: number, barrier?: number) => {
        ...
        await ensureApiReady(); // <-- key difference
    ...
    const handleSettle = useCallback((cid: string, net: number) => {
        ...
        if (!multiBuyRef.current) {
            ...
            // Strategy switching (original behavior)
            const actives = activeStrategiesRef.current;
            if (actives && actives.length > 0) {
                if (won) {
                    currentStratIndexRef.current = 0; setCurrentStratIndex(0);
                } else {
    ...
    const startBot = useCallback(() => {
        ...
        // determine base stake (strategy stake > default stake > 0.35)
        ...
        // Initialize Turbo runs only when Multi-Buy is ON
        if (multiBuy) {
            const total = isNum(turboRunsRef.current) && turboRunsRef.current > 0
            ...
            setStatus('Bot started (Switch on loss)', 'success');
        }
        ...
        scheduleNext('start');
    }, [activeStrategies, martingaleInput, market, ticks, volSwitch, setStatus, stakeInput, multiBuy]);
    ...
    return (
        <div className="flipaaaa">
            ...
            <div style={{ border: '1px solid #2b2b2b', borderRadius: 8, padding: 10, marginBottom: 12 }}>
                <div className="active-strategies" style={{ fontWeight: 700, marginBottom: 8 }}>Active strategies ({multiBuy ? 'multi-buy' : 'switch-on-loss'})</div>
            ...
            <div className="trading-container">
                ...
                <div className="trade-controls">
                    ...
                    {/* NEW: Turbo Runs input (only for Turbo mode) */}
                    ...
                    <div className="trade-control-group">
                        ...
                        <button
                            ...
                            title={multiBuy ? 'Buys all actives each round' : 'Next trade after previous settles'}
                        >
                            {isRunning ? 'ON' : 'OFF'}
                        </button>
                    </div>
            ...
            <div className="trade-status">
                ...
                <div style={{ marginTop: 6 }}>
                    ...
                    {multiBuy && isRunning && isNum(turboRunsRef.current) && (
