...
const Flipa = observer(() => {
  ...
  useEffect(() => {
    ...
    if (lastTrade.status === 'won') {
      ...
      const newStake = parseFloat((currentStake * martingale).toFixed(2));
  ...
  const buy = useCallback(async (ct: string, stakeOv?: number, marketOv?: string, durOv?: number) => {
    ...
    if (now - lastTradeTimeRef.current < 1100) {
    ...
    try {
      ...
      setTrades(t =>
        t.map(tr =>
          tr.id === tmpID ? { ...tr, id: realID, temp: false, status: 'open', profit: 0 } : tr
        )
      );
  ...
  const processTick = useCallback(async (price: number, epoch: number) => {
  ...
  }, [autoTradeEnabled, bothMode, buy, currentStake, enqueueTrade, mapContracts]);
    ...
    const id = setInterval(() => {
      setTrades(prev => prev.map(tr => {
        if (tr.status === 'pending') {
          ...
          if (age > 8000) {
            return { ...tr, status: 'error', temp: false, profit: 0 };
          }
        }
        return tr;
      }));
