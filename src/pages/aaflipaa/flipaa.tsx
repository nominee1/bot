...
export default function MultiStrategyBot() {
  ...
  const VOL_LIST = useRef<string[]>([
    'R_10', '1HZ10V', '1HZ15V', 'R_25', '1HZ25V', '1HZ30V', 'R_50', '1HZ50V', 'R_75', '1HZ75V', 'R_90', '1HZ90V', 'R_100', '1HZ100V',
  ...
  const [delayAfterSettle, setDelayAfterSettle] = useState(false);
  ...
  /* ===== BUY (throttled) for single / normal mode ===== */
  ...
  /* ===== BUY (no throttle) for TURBO / Multi-Buy same-time ===== */
  ...
  const scheduleNext = (why: 'start' | 'after_win' | 'after_loss' | 'after_error' = 'start') => {
    const run = () => {
      ...
      if (multiBuyRef.current) {
        ...
        // Turbo: fire all active strategies at (almost) the same time
        batch.forEach(b => {
  ...
  const handleSettle = useCallback((cid: string, net: number) => {
    ...
    if (!multiBuyRef.current) {
      ...
      // Strategy switching + Main Mode logic
      const actives = activeStrategiesRef.current;
      if (actives && actives.length > 0) {
        ...
        if (won) {
          // On win: reset streak and go back to first strategy
          ...
          if (switchOnLossRef.current) {
            ...
            if (nextStreak >= threshold) {
              if (mainOn) {
                ...
                if (idx < lastIdx) {
                  // Not on last strategy → behave as usual, move to next
                  const next = idx + 1;
                  currentStratIndexRef.current = next;
                  setCurrentStratIndex(next);
                  lossStreakByStratRef.current[key] = 0;
                } else {
                  // On last strategy in Main Mode: stay here on loss
                  // Just reset the streak so we don't "queue" another forced switch
                  lossStreakByStratRef.current[key] = 0;
                }
              } else {
                // Normal behaviour (no Main Mode): round-robin switch
      ...
      // Multi-mode martingale per strategy
    ...
    <div className="flipaaaa">
      ...
      <div style={{ border: '1px solid #2b2b2b', borderRadius: 8, padding: 10, marginBottom: 12 }}>
        <div className="active-strategies" style={{ fontWeight: 700, marginBottom: 8 }}>
          ...
          {multiBuy
            ? 'multi-buy'
            : `switch-on-loss${switchOnLossRef.current ? ` · ${lossesToSwitchRef.current} loss${(lossesToSwitchRef.current ?? 1) > 1 ? 'es' : ''}` : ''}`}
          {mainMode ? ' · Main mode' : ''}
          )
        </div>
        ...
        <div className="trade-controls">
          ...
          <div className="trade-control-group" style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <button
              ...
              style={{ border: '1px solid #333', background: mainMode ? '#2e7d32' : '#424242', color: '#fff', alignItems: 'center', minHeight: 36 }}
            >
          ...
          <div className="trade-control-group" style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <button
              ...
              style={{ border: '1px solid #333', background: delayAfterSettle ? '#2e7d32' : '#424242', color: '#fff', alignItems: 'center', minHeight: 36 }}
            >
            ...
            <button
              ...
              onClick={isRunning ? () => {
                ...
                if (multiBuy) {
                  ...
                  if (switchOnLossRef.current) {
                    setStatus(`Bot started (Switch on loss · ${lossesN} loss${lossesN > 1 ? 'es' : ''} to switch${mainMode ? ' · Main mode' : ''})`, 'success');
                  } else {
                    setStatus(`Bot started (No auto-switch${mainMode ? ' · Main mode' : ''})`, 'success');
                  }
                }
              ...
              title={multiBuy ? 'Buys all actives each round simultaneously' : (switchOnLoss ? 'Maintains sequence with Main Mode behaviour' : 'Manual switching')}
            >
              {isRunning ? 'ON' : 'OFF'}
            </button>
          </div>
      ...
      <div className="trade-status">
        ...
        <div style={{ marginTop: 6 }}>
          ...
          {mainMode && !multiBuy && (
            <span style={{ marginLeft: 12 }}>· Main Mode: <b>on</b></span>
          )}
          ...
          <span style={{ marginLeft: 12 }}>· Min buy gap: <b>{MIN_BUY_GAP_MS}ms (single mode)</b></span>
