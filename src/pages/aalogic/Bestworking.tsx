...
function useTickStream(symbol: string, onTick: (p: number, epoch: number) => void) {
  useEffect(() => {
    let active = true;
    ...
    const run = async () => {
      try {
        ...
        const sub = api_base.api.onMessage().subscribe(({ data }: any) => {
          ...
          onTick(data.tick.quote, data.tick.epoch);
        });
...
const SpeedBot = observer(() => {
  ...
  const [activeTickSubscription, setActiveTickSubscription] = useState<string | null>(null);
  ...
  const [martingale, setMartingale] = useState(1.25); // Changed to 1.25 as requested
  const [currentStake, setCurrentStake] = useState(0);
  const [consecutiveLosses, setConsecutiveLosses] = useState(0);
  const [baseStake, setBaseStake] = useState(0);
  ...
  const martingaleRef = useRef<HTMLInputElement>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const subs = useRef<Set<string>>(new Set());
  const prevTickRef = useRef<number | null>(null);
  const debounceTimer = useRef<NodeJS.Timeout>();
  const lastTradeTimeRef = useRef<number>(0);
  const tradeQueueRef = useRef<Array<() => Promise<void>>>([]);
  const isProcessingRef = useRef(false);
  const pendingTicksRef = useRef<Array<{ price: number, epoch: number }>>([]);
  const tickProcessingRef = useRef(false);
  const retryCountRef = useRef(0);
  const minTradeInterval = 1100;
  const maxRetries = 3;
  const currentStrategyRef = useRef(strategy); // Added to track current strategy without re-renders

  useEffect(() => {
    currentStrategyRef.current = strategy; // Keep ref in sync with state
  }, [strategy]);

  useEffect(() => {
    if (takeProfit > 0 && profitLoss >= takeProfit) {
      setAutoTradeEnabled(false);
      showStatus(`Target profit hit ($${takeProfit})`, 'success');
    } else if (stopLoss > 0 && profitLoss <= -stopLoss) {
      setAutoTradeEnabled(false);
      showStatus(`Stop loss hit ($${stopLoss})`, 'error');
    }
  }, [profitLoss, takeProfit, stopLoss]);

  useEffect(() => {
    if (stakeRef.current) {
      const stake = parseFloat(stakeRef.current.value) || 0;
  ...
  useEffect(() => {
    ...
    if (!lastTrade || martingale <= 0) return;

    if (lastTrade.status === 'won') {
      setCurrentStake(baseStake);
      setConsecutiveLosses(0);
    } else if (lastTrade.status === 'lost') {
      const newStake = parseFloat((currentStake * martingale).toFixed(2)); // Round to 2 decimal places
      setCurrentStake(newStake);
      setConsecutiveLosses(prev => prev + 1);
    }
  }, [trades[0]?.status, martingale, baseStake]);

  const getBalanceError = useCallback((error: any): { isBalanceError: boolean; isRateLimit: boolean; message: string } => {
    if (!error) return { isBalanceError: false, isRateLimit: false, message: 'Unknown error' };
    ...
    const isRateLimit = errorCode === 'RateLimit' ||
    ...
    return {
      isBalanceError,
      isRateLimit,
      message: isRateLimit ? 'Too many requests - slowing down' : isBalanceError ? rawMsg : 'Unknown error'
    };
  }, []);

  const processQueue = useCallback(async () => {
    if (isProcessingRef.current || tradeQueueRef.current.length === 0) return;
    isProcessingRef.current = true;

    try {
      const nextTrade = tradeQueueRef.current.shift();
      if (nextTrade) await nextTrade();
    } catch (error) {
      console.error('Trade queue processing error:', error);
    } finally {
      isProcessingRef.current = false;
      setTimeout(processQueue, minTradeInterval);
    }
  }, [minTradeInterval]);

  const enqueueTrade = useCallback((tradeFn: () => Promise<void>) => {
    tradeQueueRef.current.push(tradeFn);
    if (!isProcessingRef.current) {
      processQueue();
    }
  }, [processQueue]);

  const buyWithRetry = useCallback(async (ct: string, stakeOv?: number, marketOv?: string, durOv?: number) => {
    try {
      const result = await buyInternal(ct, stakeOv, marketOv, durOv);
      retryCountRef.current = 0;
      return result;
    } catch (error: any) {
      const { isRateLimit } = getBalanceError(error);
      if (isRateLimit && retryCountRef.current < maxRetries) {
        retryCountRef.current++;
        const delay = Math.pow(2, retryCountRef.current) * 1000;
        await new Promise(resolve => setTimeout(resolve, delay));
        return buyWithRetry(ct, stakeOv, marketOv, durOv);
      }
      throw error;
    }
  }, [getBalanceError]);

  const buyInternal = useCallback(async (ct: string, stakeOv?: number, marketOv?: string, durOv?: number) => {
    const now = Date.now();

    if (now - lastTradeTimeRef.current < minTradeInterval) {
      console.warn('Trade throttled - skipping execution');
      return;
    }
    lastTradeTimeRef.current = now;

    const stake = stakeOv ?? currentStake;
    ...
    try {
      ...
      setTrades(t =>
        t.map(tr =>
          tr.id === tmpID ? { ...tr, id: realID, temp: false, status: 'open' } : tr
        )
      );

      subContract(realID);
      showStatus('Trade placed ✅', 'success');
      return realID;
    } catch (e: any) {
      const errorObj = (() => {
        try {
      ...
      const { isBalanceError, isRateLimit, message } = getBalanceError(errorObj);

      if (isRateLimit) {
        setRateLimitState(prev => ({
          lastLimit: Date.now(),
          limitedCount: prev.limitedCount + 1
        }));

        if (autoTradeEnabled && rateLimitState.limitedCount > 2) {
          setAutoTradeEnabled(false);
          showStatus('Auto-trade disabled due to rate limits', 'warning');
        }
      }
      ...
      showStatus(message || 'Trade failed', isRateLimit ? 'warning' : 'error');
      return;
    }
  }, [autoTradeEnabled, currentStake, getBalanceError, rateLimitState.limitedCount]);

  const buy = useCallback(async (ct: string, stakeOv?: number, marketOv?: string, durOv?: number) => {
    return enqueueTrade(() => buyWithRetry(ct, stakeOv, marketOv, durOv));
  }, [enqueueTrade, buyWithRetry]);

  const subContract = useCallback(async (id: string) => {
  ...
  const unsubContract = useCallback(async (id: string) => {
    if (!subs.current.has(id)) return;
    try {
  ...
  const showStatus = useCallback((txt: string, type: 'info' | 'success' | 'error' | 'loading' | 'warning' = 'info') =>
    setMsg({ txt, type }), []);
  ...
  const needsDigit = useCallback((s: string) => ['matches', 'differs', 'over', 'under'].includes(s), []);

  const mapContracts = useCallback((s: string): [string, string] => ({
  ...
  const label = useCallback((ct: string) => ({
  ...
  const createTempTrade = useCallback((ct: string, stake: number, market: string, dur: number, barrier?: number) => {
  ...
  const buyBoth = useCallback(async () => {
    try {
      await Promise.all([
        buy(ctypes.left),
        buy(ctypes.right)
  ...
  }, [trades, unsubContract, showStatus, baseStake]);

  const handleWS = useCallback((d: any) => {
    ...
    if (d.error) {
      const { isBalanceError, isRateLimit, message } = getBalanceError(d);
      showStatus(message, isRateLimit ? 'warning' : isBalanceError ? 'error' : 'error');
  ...
  }, [currentSymbol, playSound, unsubContract]);
  ...
  const handleOverDigitSelect = useCallback((digit: number) => {
    setAnalysisData(prev => ({ ...prev, overDigit: digit }));
  }, []);

  const handleUnderDigitSelect = useCallback((digit: number) => {
  ...
  const processTick = useCallback(async (price: number, tickTime: number) => {
    if (!autoTradeEnabled) return;

    const dur = parseInt(durRef.current?.value || '1', 10);
    const market = marketRef.current?.value ?? '1HZ10V';
    const currentStrategy = currentStrategyRef.current; // Use ref instead of state
    const [leftContract, rightContract] = mapContracts(currentStrategy);

    enqueueTrade(async () => {
      try {
        await buyWithRetry(leftContract, currentStake, market, dur);
        if (bothMode) {
          await buyWithRetry(rightContract, currentStake, market, dur);
        }
      } catch (e) {
        console.error("Auto-trade failed:", e);
      }
    });
  }, [autoTradeEnabled, bothMode, buyWithRetry, currentStake, enqueueTrade, mapContracts]);

  const handleTick = useCallback((price: number, epoch: number) => {
    if (debounceTimer.current) clearTimeout(debounceTimer.current);
    debounceTimer.current = setTimeout(() => {
      const prev = prevTickRef.current;
      if (prev === null) { prevTickRef.current = price; return; }

      pendingTicksRef.current.push({ price, epoch });

      if (!tickProcessingRef.current) {
        ...
        setTimeout(() => {
          ...
          if (ticksToProcess.length > 0) {
            ...
            processTick(lastTick.price, lastTick.epoch);
          }
        }, 100);
      }
      ...
      if (prevTickRef.current !== null) {
        if (price > prevTickRef.current) {
          isRise = true;
        } else if (price < prevTickRef.current) {
          isRise = false;
        }
      }
  ...
  const renderEvenOddHistory = useCallback(() => {
  ...
  const renderOverUnderHistory = useCallback(() => {
    return analysisData.lastResults.slice(0, 100).map((result, index) => {
  ...
  }, [analysisData.lastResults, analysisData.overDigit, analysisData.underDigit]);

  const renderRiseFallHistory = useCallback(() => {
  ...
  }, [analysisData.lastPrice, analysisData.lastResults]);
  ...
  useEffect(() => {
    if (autoTradeEnabled && currentSymbol !== activeTickSubscription) {
      setActiveTickSubscription(currentSymbol);
    }
  }, [autoTradeEnabled, currentSymbol]);
    ...
    const initializeWebSocket = (symbol: string) => {
      ...
      wsRef.current.onmessage = (event) => {
        ...
        if (data?.msg_type === 'history') {
          ...
          handleTick(data.tick.quote, data.tick.epoch);
        }
      };
  ...
  }, [marketSelectionRef.current?.value, handleTick]);
  ...
  }, [strategy, mapContracts, needsDigit]);
  ...
  const posClass = useCallback((st: TradeStatus, p?: number) =>
    st === 'won' ? 'position-win' :
      st === 'lost' || st === 'error' ? 'position-loss' :
        'position-open', []);

  const formatTickValue = useCallback((value?: number, marketFormat?: string) => {
  ...
  const tradeStats = useMemo(() => getTradeStats(), [getTradeStats]);

  return (
    <div className="speed-app" style={{ background: ui.is_dark_mode_on ? 'var(--general-main-1)' : 'transparent' }}>
      {autoTradeEnabled && activeTickSubscription && (
        <TickSubscriber symbol={activeTickSubscription} onTick={handleTick} />
      )}
      ...
      <div className="trading-container">
        <div className="history-title">Instant Flippa</div>
        ...
        <div className="trade-controls">
          ...
          <div className="trade-control-group">
            <label>Flipper-Switch 💎</label>
            <button
              className={`auto-trade-toggle ${autoTradeEnabled ? 'on' : 'off'}`}
              onClick={() => setAutoTradeEnabled(e => !e)}
              style={{
                padding: '.4rem .8rem',
              ...
              title="Switch between markets, strategies, predictions,and martingale"
            >
        ...
        <span>Turn on Flipper-Switch to start Bot</span>

        </div>

        <div className="trade-buttons">
          <button
            className={`trade-btn even-btn ${autoTradeEnabled ? 'disabled' : ''}`}
          ...
          <button
            className={`trade-btn odd-btn ${autoTradeEnabled ? 'disabled' : ''}`}
        ...
        <div className={`trade-status status-${msg.type}`}>
          ...
          {autoTradeEnabled && <span style={{ marginLeft: '10px' }}>🔥 Market Switcher is ACTIVE</span>}
          {martingale > 0 && consecutiveLosses > 0 && (
            <span style={{ marginLeft: '10px' }}>
              Martingale: {consecutiveLosses} loss(es), current stake: ${currentStake.toFixed(2)}
            </span>
          )}
        </div>
...
const TickSubscriber = ({ symbol, onTick }: { symbol: string; onTick: (p: number, epoch: number) => void }) => {
