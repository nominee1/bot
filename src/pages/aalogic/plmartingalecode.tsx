...
const SpeedBot = observer(() => {
  ...
  // Martingale state with pre-calculated stakes
  const martingaleState = useRef({
    active: false,
    consecutiveLosses: 0,
    baseStake: 2,
    stakes: [2] as number[], // Pre-calculated stakes array
    currentStep: 0
  });
  ...
  const isTradeProcessing = useRef(false); // Lock for trade processing
  ...
  // Pre-calculate Martingale stakes
  const calculateMartingaleStakes = (baseStake: number, multiplier: number, maxSteps: number) => {
    const stakes = [baseStake];
    let current = baseStake;
    for (let i = 1; i <= maxSteps; i++) {
      current *= multiplier;
      stakes.push(parseFloat(current.toFixed(2))); // Round to 2 decimals
    }
    return stakes;
  };
  ...
  const resetMartingale = () => {
    const baseStake = parseFloat(stakeRef.current?.value || '2');
    martingaleState.current = {
      active: martingaleEnabled,
      consecutiveLosses: 0,
      baseStake,
      stakes: calculateMartingaleStakes(baseStake, martingaleMultiplier, martingaleMaxSteps),
      currentStep: 0
    };
  };

  // Update Martingale state after trade result
  const handleMartingaleResult = (won: boolean, tradeId: string) => {
    if (!martingaleEnabled) return;

    if (won) {
      resetMartingale();
    } else {
      martingaleState.current.consecutiveLosses++;
      if (martingaleState.current.consecutiveLosses > martingaleMaxSteps) {
        resetMartingale();
      } else {
        martingaleState.current.currentStep = martingaleState.current.consecutiveLosses;
      }
    }

    // Mark trade as counted to prevent re-processing
    setTrades(prev => prev.map(tr => 
      tr.id === tradeId ? { ...tr, counted: true } : tr
    ));
  };

  const createTempTrade = (ct: string, stake: number, market: string, dur: number, barrier?: number, isAuto?: boolean) => {
  ...
  const buy = async (ct: string, isAuto = false, stakeOv?: number, marketOv?: string, durOv?: number) => {
    if (isTradeProcessing.current) {
      console.log('Trade processing, skipping new trade');
      return null;
    }

    isTradeProcessing.current = true;
    let stake = stakeOv ?? parseFloat(stakeRef.current?.value || '2');
    const dur = durOv ?? parseInt(durRef.current?.value || '1', 10);
    const market = marketOv ?? marketRef.current?.value ?? '1HZ10V';

    // Apply Martingale stake if enabled and auto-trading
    if (martingaleEnabled && isAuto) {
      stake = martingaleState.current.stakes[martingaleState.current.currentStep];
    }
    ...
    if (['DIGITOVER', 'DIGITUNDER', 'DIGITMATCH', 'DIGITDIFF'].includes(ct)) {
      ...
      if (isNaN(d)) { 
        ...
        isTradeProcessing.current = false;
        throw new Error('digit'); 
      }
      barrier = d.toString();
    }

    const tmpID = createTempTrade(ct, stake, market, dur, barrier ? +barrier : undefined, isAuto);
  ...
  const toggleAutoTrading = () => {
    ...
    if (newAutoTradingState && martingaleEnabled) {
      resetMartingale();
    }
  ...
  const toggleMartingale = () => {
    const newMartingaleState = !martingaleEnabled;
    setMartingaleEnabled(newMartingaleState);

    if (newMartingaleState) {
      resetMartingale();
    }

    showStatus(newMartingaleState ? 'Martingale enabled' : 'Martingale disabled', 'info');
  };
  ...
  const handlePOC = (c: any) => {
    setTrades(prev => prev.map(tr => {
      ...
      if (finished && !tr.counted) {
        ...
        if (tr.isAutoTrade && martingaleEnabled) {
          handleMartingaleResult(net >= 0, tr.id);
        }
      } else {
  ...
  const handleTX = (tx: TTransaction) => {
    setTrades(prev => prev.map(tr => {
      if (tr.id !== tx.contract_id || tr.counted) return tr;
  ...
  const handleTick = (val: number) => {
    ...
    debounceTimer.current = setTimeout(() => {
      ...
      // Auto-trading: Place trade on tick if not processing
      if (autoTradeActiveRef.current && !isTradeProcessing.current) {
        const contractType = ctypes.left;
        buy(contractType, true).catch(() => {
          setAutoTrading(false);
          autoTradeActiveRef.current = false;
          showStatus('Auto-trading stopped due to error', 'error');
        });
      }
  ...
  // Update Martingale stakes when multiplier or max steps change
  useEffect(() => {
    if (martingaleEnabled) {
      resetMartingale();
    }
  }, [martingaleMultiplier, martingaleMaxSteps]);
        ...
        <div className="trade-controls">
          ...
          <div className="trade-control-group">
            ...
            <input type="number" className="trade-input" defaultValue="2"
              ...
              onChange={() => {
                if (martingaleEnabled) resetMartingale();
              }} />
          </div>
            ...
            <>
              ...
              <div className="trade-control-group">
                ...
                <div className="trade-input">
                  {martingaleState.current.stakes[martingaleState.current.currentStep].toFixed(2)} USD
                </div>
              </div>
              ...
              <div className="trade-control-group">
                <label>Step</label>
                <div className="trade-input">
                  {martingaleState.current.currentStep} / {martingaleMaxSteps}
                </div>
              </div>
            </>
          )}
