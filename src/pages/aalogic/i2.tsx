...
const SpeedBot = observer(() => {
  ...
  const handleTick = (val: number) => {
    ...
    debounceTimer.current = setTimeout(() => {
      ...
      // Auto-trading logic - trigger trade based on selected strategy
      if (autoTradeActiveRef.current) {
        const contractType = strategy === 'odd' ? 'DIGITODD' :
                            strategy === 'matches' ? 'DIGITMATCH' :
                            strategy === 'differs' ? 'DIGITDIFF' :
                            strategy === 'over' ? 'DIGITOVER' :
                            strategy === 'under' ? 'DIGITUNDER' :
                            strategy === 'rise' ? 'CALL' :
                            strategy === 'fall' ? 'PUT' :
                            'DIGITEVEN'; // Default to even if strategy is not recognized
  ...
  return (
    <div className="bots-app" style={{ background: ui.is_dark_mode_on ? 'var(--general-main-1)' : 'transparent' }}>
