...
const SpeedBot = observer(() => {
  ...
  // Martingale logic to calculate next stake
  const getNextMartingaleStake = (currentStake: number, multiplier: number) => {
    return currentStake * multiplier;
  };

  // Reset martingale after a win
  const resetMartingale = () => {
    martingaleState.current = {
      active: false,
      currentStep: 0,
      baseStake: parseFloat(stakeRef.current?.value || '2'),
      currentStake: parseFloat(stakeRef.current?.value || '2'),
      consecutiveLosses: 0
    };
  };

  // Handle martingale logic after trade result
  const handleMartingaleResult = (won: boolean) => {
    ...
    if (won) {
      ...
      if (martingaleState.current.consecutiveLosses >= martingaleMaxSteps) {
        ...
        martingaleState.current.currentStep++;
        martingaleState.current.currentStake = getNextMartingaleStake(
          martingaleState.current.currentStake,
  ...
  const buy = async (ct: string, isAuto = false, stakeOv?: number, marketOv?: string, durOv?: number) => {
    let stake = stakeOv ?? parseFloat(stakeRef.current?.value || '0');
    const dur = durOv ?? parseInt(durRef.current?.value || '1', 10);
    const market = marketOv ?? marketRef.current?.value ?? '1HZ10V';
  
    // Apply martingale if enabled and it's an auto trade
    if (martingaleEnabled && isAuto) {
      stake = martingaleState.current.currentStake;
    }
    ...
    try {
      const resp = await api_base.api.send({
        buy: 1,
        price: stake, // This should be the correct parameter for stake
        parameters: {
          amount: stake, // Some APIs might expect this instead
          basis: 'stake', 
          currency: 'USD',
          contract_type: ct, 
          duration: dur, 
  ...
  const toggleAutoTrading = () => {
    ...
    // Reset martingale when auto trading starts
  ...
  const toggleMartingale = () => {
    ...
    // Reset martingale state when enabling
  ...
  const handlePOC = (c: any) => {
    setTrades(prev => prev.map(tr => {
      ...
      if (finished) {
        ...
        // Handle martingale logic for auto trades
        if (tr.isAutoTrade && martingaleEnabled) {
          handleMartingaleResult(net >= 0);
        }
      } else {
  ...
  const handleTick = (val: number) => {
    ...
    debounceTimer.current = setTimeout(() => {
      ...
      if (['JD10', 'JD25', 'JD50', 'JD75', 'JD100'].includes(currentMarket)) {
        tickString = val.toFixed(2); // Jump indices use 2 decimal places
      ...
      // Auto-trading logic - trigger trade on every tick when auto-trading is active
      if (autoTradeActiveRef.current) {
        const contractType = ctypes.left; // Using left strategy for auto-trading
        buy(contractType, true).catch(() => {
          // Stop auto-trading on error
            ...
            <>
              ...
              <div className="trade-control-group">
                ...
                <div className="trade-input">
                  {martingaleState.current.currentStake.toFixed(2)} USD
                </div>
              </div>
