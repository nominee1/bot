...
const CopyTradingHub: React.FC = () => {
  ...
  const [tEmail, setTEmail] = useState(''); // 👈 NEW
  ...
  const [stmHasMore, setStmHasMore] = useState(false); // 🚫 never loading more
  const [stmLoading, setStmLoading] = useState(false);
  const [stmErr, setStmErr] = useState<string | null>(null);

  // Use "today from 3:00 AM (Nairobi) → now" everywhere for statements window label
  ...
  /** Statements modal */
  const openStatements = (trader: Trader) => {
    // ✅ Just open modal, DO NOT fetch statements
    setStatementModalFor(trader);
    setStmItems([]); // keep list empty
    setStmOffset(0);
    setStmHasMore(false); // no pagination
    setStmErr(null);
    setStmLoading(false);
  };

  /** ====== Client-side summary for CURRENTLY VISIBLE stmItems (empty, so blank) ====== */
  type VisibleSummary = {
    start_balance?: number | null;
    end_balance?: number | null;
    net_pl?: number | null;
  ...
  const visibleSummary: VisibleSummary = useMemo(() => {
    ...
    // kept logic in case you ever re-enable stmItems
    ...
    <div className="cth-root" data-theme={theme}>
      ...
      <header className="topbar glass">
        ...
        <div className="topbar__actions">
          ...
          <button className="btn" onClick={() => loadTraders()}>
            Refresh
          </button>
        </div>
      </header>
      ...
      <div className="ct-hub__grid">
        ...
        <section className="card card--form glass is-hidden-for-now">
          ...
          <p className="muted small">
            Requires Deriv token with <strong>Read</strong>, <strong>Trade</strong>, and{' '}
            ...
            onSubmit={async (e) => {
              ...
              try {
                const res = await fetch(`${API_BASE}/traders`, {
                  ...
                  body: JSON.stringify({
                    username: tUser.trim(),
                    email: tEmail.trim() || undefined, // 👈 send email
        ...
        <section className="card card--list glass">
          ...
          {loadingList ? (
            ...
            <div className="empty">
              ...
              <div className="muted">
                “{FEATURED_USERNAME}” is not registered yet. <strong>Admin:</strong> register the trader to enable
                copytrading.
              </div>
              <button className="btn ghost" onClick={() => loadTraders()}>
                Retry
              </button>
            </div>
          ) : (
                ...
                <div className="trader-card__main">
                  ...
                  <div className="copy-inline">
                    <input
                      ...
                      onChange={(e) =>
                        setCopyUsernameByTrader((prev) => ({ ...prev, [card.id]: e.target.value }))
                      }
                    />
                    <button
                      className="btn"
                      disabled={
                        !card?.id || !(copyUsernameByTrader[card.id] || '').trim() || !!copyBusyByTrader[card.id]
                      }
                    ...
                    <button
                      className="btn ghost"
                      disabled={
                        !card?.id || !(copyUsernameByTrader[card.id] || '').trim() || !!stopBusyByTrader[card.id]
                      }
                  ...
                  <div className="muted xsmall pay-explainer">
                    This <strong>trader</strong> charges <strong>USD 20</strong> for services and a minimum balance of
                    Usd 50. To copy trader copier token Must have Payment and Admin Permissions.
                  </div>
        ...
        <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Trader statements">
          <div className="modal">
            ...
            <div className="modal__body">
              ...
              {/* ======== VISIBLE SUMMARY BAR (values will be blank because stmItems is empty) ======== */}
              {!stmErr && (
              ...
              {/* ======== STATEMENTS TABLE (ONLY HEADER, NO ROWS) ======== */}
              {!stmErr && (
                <>
                  ...
                  {/* stmItems is always [], so no rows are displayed */}
                  ...
                  <div className="modal__foot modal__foot--spread">
                    ...
                    <div className="actions">
                      {/* 🚫 Load more disabled permanently; no fetch */}
          ...
          <div className="modal otp-modal">
            ...
            <div className="modal__body">
              ...
              <div className="muted xsmall">
                A code was sent to your Deriv email after you initiated payment. Paste it here to complete the withdrawal
                to our Payment Agent.
              </div>
            </div>
