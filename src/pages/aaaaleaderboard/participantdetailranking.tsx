...
async function computeWindowMetrics(
  ...
  const netPL =
    typeof endBal === 'number' && typeof baselineBal === 'number' ? endBal - baselineBal : 0;
        ...
        <div className="modal__content">
          ...
          <section>
            ...
            <ul>
              ...
              <li>Ranking: by <strong>Return %</strong> from your baseline balance.</li>
          ...
          <section>
            ...
            <ul>
              ...
              <li>Only trades made on <strong>denarapro.com</strong> will qualify for prizing.</li>
...
const ParticipantsLeaderboardMerged = ({
  ...
  const computeUserStat = useCallback(
    async (u: string): Promise<UserStat> => {
      ...
      try {
        ...
        const returnPct =
          typeof metrics.baselineBal === 'number' && metrics.baselineBal > 0 && metrics.trades > 0
        ...
        const stat: UserStat =
          msg.startsWith('Non-USD')
            ? { status: 'skip', reason: msg, turnover: 0 }
            : { status: 'error', reason: msg, turnover: 0 };
        updateStat(u, stat);
    ...
    (async () => {
      ...
      for (let i = 0; i < users.length; i++) {
        ...
        try {
          ...
          updateStat(u, { status: 'error', reason: e?.message || 'Timed out', turnover: 0 });
        }
        if (!mounted) break;
  ...
  }, [users, startMs, endMs, computeUserStat]);
  ...
  // low-level page fetch used for initial load and scrolling
  const fetchStatementsPage = useCallback(
    async (pageOffset: number) => {
      if (!api_base?.api || !selectedUser) return;
    ...
    [selectedUser, startMs, endMs]
  );

  // fetch next page for infinite scroll
  const fetchPage = useCallback(async () => {
    if (!authorized || loading || !hasMore || !selectedUser) return;
  ...
  // detail load — aligned to the original working flow
  useEffect(() => {
    let cancelled = false;
    ...
    // intentionally kept narrow like the working version
  ...
  const sortedUsers: Participant[] = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      const ta = sa?.trades ?? -Infinity;
      const tb = sb?.trades ?? -Infinity;
  ...
  const top3 = sortedUsers.slice(0, 3).filter(u => typeof stats[u.username]?.returnPct === 'number');
        ...
        <div className="leaderboard-hero__content">
          <div className="leaderboard-hero__copy">
            ...
            <div className="leaderboard-hero__actions">
              ...
              {/* <button className="btn btn--ghost" onClick={scrollToRegister}>
                Register Now
              </button> */}
            </div>
          </div>
      ...
      {top3.length > 0 && (
        <div className="winners-banner">
          ...
          <div className="winners-text">
            ...
            <div className="sub">Rankings are based on tournament return percentage.</div>
          </div>
        </div>
      )}
          ...
          <div className="stm-summary">
            ...
            <div className="stm-summary__metrics">
              ...
              <div className="metric">
                ...
                <div
                  className={`m-value ${detailSummary?.returnPct !== null && (detailSummary?.returnPct ?? 0) >= 0 ? 'pos' : 'neg'
                    }`}
                >
