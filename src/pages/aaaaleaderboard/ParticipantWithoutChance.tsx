        ...
        <div className="modal__content">
          ...
          <section>
            ...
            <ul>
              ...
              <li>
                Minimum ranking balance: <strong>$10 USD</strong> starting capital.
              </li>
...
const ParticipantsLeaderboardMerged = ({
  ...
  const computeUserStat = useCallback(
    async (u: string): Promise<UserStat> => {
      ...
      if (u === UNCHAINED_USERNAME) {
        const isRankEligible = UNCHAINED_TARGET_START_BAL >= MIN_RANKING_START_BALANCE;
        const stat: UserStat = {
          status: isRankEligible ? 'ok' : 'skip',
          reason: isRankEligible
            ? undefined
            : `Minimum starting balance for ranking is ${MIN_RANKING_START_BALANCE} USD`,
          startBal: UNCHAINED_TARGET_START_BAL,
          endBal: UNCHAINED_TARGET_END_BAL,
          netPL: UNCHAINED_NET_PL,
          trades: UNCHAINED_SYN_TRADES,
          returnPct: isRankEligible ? (UNCHAINED_NET_PL / UNCHAINED_TARGET_START_BAL) * 100 : null,
      ...
      try {
        ...
        const isRankEligible =
          typeof metrics.baselineBal === 'number' && metrics.baselineBal >= MIN_RANKING_START_BALANCE;

        const returnPct =
          isRankEligible && typeof metrics.baselineBal === 'number' && metrics.baselineBal > 0 && metrics.trades > 0
  ...
  const rankedUsers = useMemo(() => {
    const eligible = users.filter(u => stats[u.username]?.isRankEligible);
  ...
  const sortedUsers: Participant[] = useMemo(() => {
    ...
    const unranked = users.filter(u => !rankedNames.has(u.username));
      ...
      {top3.length > 0 && (
        <div className="winners-banner">
          ...
          <div className="winners-text">
            ...
            <div className="sub">
              All traders appear in the list. Only traders with a minimum starting balance of $10 USD receive an official rank.
            </div>
          </div>
        </div>
      )}
