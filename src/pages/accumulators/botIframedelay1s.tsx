...
const BotIframe = observer(() => {
  ...
  const durRef = useRef<HTMLSelectElement & HTMLInputElement>(null as any);
  ...
  const nextTickQueueRef = useRef<NextTickJob[]>([]); // ← jobs waiting for next tick
  ...
  const createTempTrade = (ct: string, stake: number, market: string, dur: number, barrier?: number, isBulk?: boolean, bulkId?: string) => {
  ...
  /**
   * Actual API buy – called only when a tick arrives and a job is in the queue
   */
  const executeBuyNow = async (
    ...
    const dur = durOv ?? parseInt((durRef.current as any)?.value || '1', 10);
  ...
  /**
   * Public buy() – now just ARMS the next tick.
   * The actual buy is executed in handleTick when the very next tick comes in.
   */
  const buy = (
    ct: string,
    isBulk = false,
    ...
    return new Promise((resolve, reject) => {
      ...
      showStatus('Armed – waiting for next tick…', 'info');
    });
  };
  ...
  const startBulk = (ct: string) => {
    ...
    const duration = parseInt((durRef.current as any)?.value || '1', 10);
  ...
  const handleTick = (val: number) => {
    ...
    debounceTimer.current = setTimeout(() => {
      ...
      // 🔥 Execute any jobs that were armed before this tick.
      if (nextTickQueueRef.current.length > 0) {
        const jobs = [...nextTickQueueRef.current];
        nextTickQueueRef.current = [];
        jobs.forEach(job => {
          executeBuyNow(
            job.ct,
            job.isBulk,
