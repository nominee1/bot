...
/* =========================================================
   ✅ SAFEST Entry Point Auto-Fill (NO suggestions UI)
   - Look at last 100 ticks
   - For each entry digit D (0-9), evaluate next-tick outcome
   - Choose digit with LOWEST loss% (safest)
   ========================================================= */
type EntryScore = {
  digit: number;
  seen: number;
...
const needsBarrierForContract = (ct: string) =>
  ['DIGITOVER', 'DIGITUNDER', 'DIGITMATCH', 'DIGITDIFF'].includes(ct);

const evalWinForNextTick = (
  ct: string,
  barrier: number | null,
  cur: { digit: number; price: number },
  next: { digit: number; price: number }
): boolean | null => {
  switch (ct) {
    case 'DIGITEVEN':
      return next.digit % 2 === 0;
    case 'DIGITODD':
      return next.digit % 2 === 1;

    case 'DIGITMATCH':
      return barrier === null ? null : next.digit === barrier;
    case 'DIGITDIFF':
      return barrier === null ? null : next.digit !== barrier;
    case 'DIGITOVER':
      return barrier === null ? null : next.digit > barrier;
    ...
    // Rise/Fall: use PRICE direction from entry tick to next tick
    case 'CALL':
      return next.price > cur.price; // equal => lose
    case 'PUT':
      return next.price < cur.price; // equal => lose
...
const computeEntryScores = (
  pointsChrono: Array<{ digit: number; price: number }>,
  ct: string,
  barrier: number | null,
  minSeen = 3
): EntryScore[] => {
  const seen = Array(10).fill(0);
  const wins = Array(10).fill(0);
  const losses = Array(10).fill(0);

  for (let i = 0; i < pointsChrono.length - 1; i++) {
    const cur = pointsChrono[i];
    const next = pointsChrono[i + 1];

    const entryDigit = cur.digit;
    const res = evalWinForNextTick(ct, barrier, cur, next);
    if (res === null) continue;

    seen[entryDigit]++;
    if (res) wins[entryDigit]++;
    else losses[entryDigit]++;
  }

  const out: EntryScore[] = Array.from({ length: 10 }).map((_, d) => {
    const s = seen[d];
    const w = wins[d];
    const l = losses[d];
    return {
  ...
  // Safest = lowest lossRate, then higher samples
  return out
    .filter(x => x.seen >= minSeen)
    .sort((a, b) => a.lossRate - b.lossRate || b.seen - a.seen || b.winRate - a.winRate || a.digit - b.digit);
};

const combineScoresBoth = (left: EntryScore[], right: EntryScore[]): EntryScore[] => {
  const L = new Map(left.map(x => [x.digit, x]));
  const R = new Map(right.map(x => [x.digit, x]));
  const digits = Array.from({ length: 10 }).map((_, d) => d);

  const out: EntryScore[] = digits.map(d => {
    const a = L.get(d);
    const b = R.get(d);

    const seen = (a?.seen ?? 0) + (b?.seen ?? 0);
    const wins = (a?.wins ?? 0) + (b?.wins ?? 0);
    const losses = (a?.losses ?? 0) + (b?.losses ?? 0);

    // conservative: avoid digits that are bad for either side
    const lossRate = Math.max(a?.lossRate ?? 1, b?.lossRate ?? 1);
    const winRate = seen ? wins / seen : 0;

    return { digit: d, seen, wins, losses, winRate, lossRate };
  });
...
const BotIframe = observer(() => {
  ...
  // ✅ AUTO controls (Entry Point is auto-filled when AUTO is ON)
  const [autoOn, setAutoOn] = useState(false);
  const [autoEntryDigit, setAutoEntryDigit] = useState<string>('2'); // always kept valid 0-9 while autoOn

  // ✅ Prediction (controlled) so auto-entry recomputes instantly
  ...
  // which contract should drive the auto-entry (changes when you arm left/right/both/bulk)
  const autoPrefRef = useRef<'left' | 'right' | 'both'>('left');

  useEffect(() => {
    autoOnRef.current = autoOn;
  }, [autoOn]);

  // keep autoEntryRef always synced
  useEffect(() => {
    const s = autoEntryDigit.trim();
    const n = Number(s);
    autoEntryRef.current = Number.isFinite(n) ? Math.min(9, Math.max(0, Math.trunc(n))) : 2;
  }, [autoEntryDigit]);
  ...
  const computeSafestEntryDigit = useCallback(
    (pref: 'left' | 'right' | 'both'): number | null => {
      const N = 100;
      const slice = analysisData.lastResults.slice(0, N);
      const pointsChrono = [...slice].reverse().map(x => ({ digit: x.digit, price: Number(x.price) }));
      if (pointsChrono.length < 2) return null;

      const barrier = Number.isFinite(predictionRef.current) ? predictionRef.current : null;

      const leftNeeds = needsBarrierForContract(ctypes.left);
      const rightNeeds = needsBarrierForContract(ctypes.right);

      const leftScores = leftNeeds && barrier === null ? [] : computeEntryScores(pointsChrono, ctypes.left, barrier, 3);
      const rightScores = rightNeeds && barrier === null ? [] : computeEntryScores(pointsChrono, ctypes.right, barrier, 3);

      if (pref === 'both') {
        const bothScores = combineScoresBoth(leftScores, rightScores);
        return bothScores[0]?.digit ?? null;
      }
      if (pref === 'right') return rightScores[0]?.digit ?? null;

      // default left
      return leftScores[0]?.digit ?? null;
    },
    [analysisData.lastResults, ctypes.left, ctypes.right]
  );

  const setAutoEntryDigitSafe = (d: number) => {
    const clamped = Math.min(9, Math.max(0, Math.trunc(d)));
    autoEntryRef.current = clamped; // immediate
    const s = String(clamped);
    setAutoEntryDigit(prev => (prev === s ? prev : s));
  };

  // ✅ Whenever AUTO is ON, keep entry point auto-filled (safest digit) as data / contract changes
  useEffect(() => {
    if (!autoOn) return;

    // if bothMode toggled ON, prefer 'both'
    const pref: 'left' | 'right' | 'both' = bothMode ? 'both' : autoPrefRef.current;

    const best = computeSafestEntryDigit(pref);
    if (best === null) {
      // fallback: keep current if valid else 2
      const cur = autoEntryRef.current;
      const fallback = Number.isFinite(cur) ? cur : 2;
      setAutoEntryDigitSafe(fallback);
      return;
    }

    setAutoEntryDigitSafe(best);
  }, [
    autoOn,
    bothMode,
    strategy,
  ...
  const armOrBuy = (side: 'left' | 'right') => {
    ...
    // set preference based on what you are arming (so entry adjusts if you arm the other side)
    autoPrefRef.current = bothMode ? 'both' : side;

    // force recompute immediately so the armed message uses the correct digit
    const bestNow = computeSafestEntryDigit(autoPrefRef.current);
    if (bestNow !== null) setAutoEntryDigitSafe(bestNow);

    const ep = autoEntryRef.current;
    if (!Number.isFinite(ep) || ep < 0 || ep > 9) {
      showStatus('AUTO: could not compute a safe Entry Point yet', 'warning');
      return;
    }
    ...
    showStatus(`AUTO armed → safest entry digit ${ep}`, 'info');
  };

  // ✅ bulk buttons: if AUTO is ON => arm, else start bulk immediately
  const armOrStartBulk = (ct: string, which: 'left' | 'right') => {
    if (!autoOnRef.current) return startBulk(ct);

    autoPrefRef.current = bothMode ? 'both' : which;
    ...
    autoBusyRef.current = false;
    showStatus(`AUTO armed → BULK will start on safest digit ${ep}`, 'info');
  };
  ...
  const renderRiseFallHistory = () => {
    ...
    if (filteredResults.length === 0) {
      return <div className="no-results-message">{analysisData.lastResults.length === 0 ? 'Waiting for first price data...' : 'No price changes detected'}</div>;
    }
    ...
    <div className="bot-app" style={{ background: ui.is_dark_mode_on ? 'var(--general-main-1)' : 'transparent' }}>
      ...
      <div className="trading-container">
        ...
        <div className="trade-controls">
          ...
          <div className="trade-control-group">
            ...
            <button
              onClick={() => {
                ...
                // prefer BOTH if bothMode, else default to left (strategy side)
                autoPrefRef.current = bothMode ? 'both' : 'left';
              ...
              title="When ON, Entry Point is auto-filled (safest digit) and triggers only when LIVE tick matches it"
            >
              {autoOn ? 'ON' : 'OFF'}
            </button>
          </div>
          ...
          {autoOn && (
            <div className="trade-control-group">
              <label>Entry Point (auto)</label>
              <input
                type="text"
                className="trade-input"
                value={autoEntryDigit}
                readOnly
                title="Safest Entry Digit (lowest loss%) auto-calculated from last 100 ticks"
              />
            </div>
          )}
        </div>
        ...
        <div className="trade-buttons">
          <button className="trade-btn even-btn" onClick={() => armOrStartBulk(ctypes.left, 'left')}>
            <span className="button-icon">{contractIcons[ctypes.left] || null}</span>
            Bulk {label(ctypes.left)}
          </button>
          <button className="trade-btn odd-btn" onClick={() => armOrStartBulk(ctypes.right, 'right')}>
            <span className="button-icon">{contractIcons[ctypes.right] || null}</span>
