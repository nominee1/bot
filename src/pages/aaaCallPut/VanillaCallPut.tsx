...
type TradeMode = 'vanilla' | 'touch' | 'multiplier';
type VanillaDir = 'CALL' | 'PUT';
type TouchDir = 'TOUCH' | 'NO_TOUCH';
type MultDir = 'UP' | 'DOWN';
...
type QuoteResult = {
  ...
  askPrice: number;
  payout: number;
  strikeOrBarrier: number; // vanilla strike OR touch target OR NaN for multiplier
  longcode: string;
};
...
type TAnyContract = {
  ...
  profit_line?: number; // vanilla strike OR touch target (NaN for multiplier)
...
// ✅ default options; user can always type custom
const MULTIPLIER_OPTIONS = [5, 10, 20, 30, 40, 50, 100, 200];


const VanillaCallPut = observer(() => {
  const { tradingSocketGeneration } = useApiBase();
  const { ui } = useStore();
  const [symbol, setSymbol] = useState('R_100');

  // ✅ NEW: 3-mode selector
  const [mode, setMode] = useState<TradeMode>('multiplier');
  ...
  const [multDir, setMultDir] = useState<MultDir>('UP');

  const [amount, setAmount] = useState<number | ''>(10);
  const [currency] = useState('USD');
  const [basis] = useState<'stake' | 'payout'>('stake');

  // vanilla/touch duration
  const [duration, setDuration] = useState<number | ''>(5);
  const [durationUnit, setDurationUnit] = useState<'t' | 'm' | 'h' | 'd'>('m');

  // ✅ multiplier params
  // user can select OR type custom using the same dropdown+input pattern
  const [multiplierChoices, setMultiplierChoices] = useState<string[]>(MULTIPLIER_OPTIONS.map(String));
  const [multiplierStr, setMultiplierStr] = useState<string>('40');
  const [multiplierSelect, setMultiplierSelect] = useState<string>('40');
  const [multiplierInput, setMultiplierInput] = useState<string>('40');

  const [tp, setTp] = useState<number | ''>('');
  const [sl, setSl] = useState<number | ''>('');

  // shared barrier (vanilla & touch)
  ...
  const contractType = useMemo(() => {
    ...
    if (mode === 'touch') return touchDir === 'TOUCH' ? 'ONETOUCH' : 'NOTOUCH';
    return multDir === 'UP' ? 'MULTUP' : 'MULTDOWN';
  }, [mode, vanillaDir, touchDir, multDir]);

  const displayDir = useMemo(() => {
    if (mode === 'vanilla') return vanillaDir;
    if (mode === 'touch') return touchDir === 'TOUCH' ? 'TOUCH' : 'NO TOUCH';
    return multDir === 'UP' ? 'UP' : 'DOWN';
  }, [mode, vanillaDir, touchDir, multDir]);
  ...
  // vanilla strike OR touch target (NaN for multiplier)
  ...
  // remembers last “touch key” for barrier sync
  const lastTouchKeyRef = useRef<string>('');
  ...
  const parsedMultiplier = useMemo(() => {
    const n = Number(String(multiplierStr ?? '').trim());
    return Number.isFinite(n) && n > 0 ? n : NaN;
  }, [multiplierStr]);

  /**
   * Quote request for Vanilla + Touch + Multiplier
   * - Vanilla/Touch uses barrier + duration
   * - Multiplier uses multiplier (+ optional tp/sl via limit_order)
   */
    ...
    async (reason: string): Promise<QuoteResult | null> => {
      ...
      if (mode === 'multiplier') {
        payload = {
          ...
          basis,
          currency,
          contract_type: contractType, // MULTUP / MULTDOWN
          multiplier: parsedMultiplier,
          ...(tp !== '' || sl !== ''
            ? {
              limit_order: {
                ...(tp !== '' ? { take_profit: clampNum(Number(tp), 0.01, 1_000_000) } : {}),
                ...(sl !== '' ? { stop_loss: clampNum(Number(sl), 0.01, 1_000_000) } : {}),
              },
            }
            : {}),
        };
        ...
        payload = {
          ...
          basis,
          currency,
          contract_type: contractType, // VANILLA / ONETOUCH / NOTOUCH
          duration: dur,
      ...
      applyDerivSessionMarketField(payload as Record<string, unknown>, symbol);

      try {
        // quick guard for multiplier
        if (mode === 'multiplier' && !Number.isFinite(parsedMultiplier)) {
          throw new Error('Invalid multiplier. Select or type a valid number (e.g. 10, 20, 50).');
        }
        ...
        if (mode === 'multiplier') {
          // multiplier has no barrier choices/strike
          setBarrierChoices([]);
          setProposalStrike(NaN);
        } else {
          // barrier choices
          if (bc.length) {
            ...
            if (barrierSelect !== CUSTOM_BARRIER) setBarrierChoices([]);
          }
        ...
        // barrier auto-populate for vanilla/touch
        if (mode !== 'multiplier') {
  ...
  const buy = useCallback(async () => {
    ...
    try {
      ...
      // vanilla/touch needs strike/target; multiplier doesn't
      if (mode !== 'multiplier' && !Number.isFinite(q.strikeOrBarrier)) {
        throw new Error('No strike/target yet. Wait for quote.');
      }
      ...
      const label =
        mode === 'vanilla'
          ? `Vanilla ${displayDir}`
          : mode === 'touch'
            ? `Touch/No Touch ${displayDir}`
            : `Multiplier ${displayDir} x${Number.isFinite(parsedMultiplier) ? parsedMultiplier : ''}`;

      show(`Bought ${label}: ${contract_id}`, 'success');
  ...
  }, [moveToRecent, show, unsubscribePOC, tradingSocketGeneration]);
  ...
  // auto-populate touch barriers per symbol + duration + unit + touchDir
  useEffect(() => {
    if (mode !== 'touch') return;
    if (barrierSelect === CUSTOM_BARRIER) return;

    const dur = typeof duration === 'number' ? duration : 5;
    const key = `${symbol}|${dur}|${durationUnit}|${touchDir}`;

    if (lastTouchKeyRef.current === key) return;
    lastTouchKeyRef.current = key;

    setBarrierChoices([]);
    setBarrier(prev => (prev && prev.trim() ? prev : '+0.00'));
    setBarrierSelect(prev => (prev && prev.trim() ? prev : '+0.00'));
    setBarrierInput(prev => (prev && prev.trim() ? prev : '+0.00'));

    requestProposal('touch barrier sync').catch(() => { });
  }, [mode, symbol, duration, durationUnit, touchDir, barrierSelect, requestProposal]);
  ...
  const customBarrierMode = barrierSelect === CUSTOM_BARRIER;

  const multiplierOptions = useMemo(() => {
    const base = multiplierChoices.length ? multiplierChoices : MULTIPLIER_OPTIONS.map(String);
    return [...base, CUSTOM_BARRIER];
  }, [multiplierChoices]);

  const customMultiplierMode = multiplierSelect === CUSTOM_BARRIER;

  const barrierLabel = mode === 'vanilla' ? 'Offset (Barrier)' : mode === 'touch' ? 'Barrier (Auto per Duration)' : '—';

  return (
    <div className={`vanilla-page${ui.is_dark_mode_on ? ' vanilla-page--dark' : ''}`}>
      ...
      <div className="vc-header">
        <div className="title">
          <h3>{mode === 'vanilla' ? 'Vanilla Call / Put' : mode === 'touch' ? 'Touch / No Touch' : 'Multiplier'}</h3>
          <div className="sub">
        ...
        <div className="dir-toggle" style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <button
            className={mode === 'multiplier' ? 'active' : ''}
            onClick={() => {
              setMode('multiplier');
              lastTouchKeyRef.current = '';
            }}
          >
            MULTIPLIER
          </button>
      ...
      <div className="vc-header" style={{ paddingTop: 0 }}>
        {mode === 'vanilla' ? (
          ...
          <div className="dir-toggle">
            <button className={multDir === 'UP' ? 'active' : ''} onClick={() => setMultDir('UP')}>
              UP
            </button>
            <button className={multDir === 'DOWN' ? 'active' : ''} onClick={() => setMultDir('DOWN')}>
              DOWN
            </button>
          </div>
        )}
        ...
        <div className="vc-card">
          <div className="row">
            ...
            {mode !== 'multiplier' ? (
              ...
              <>
                {/* Multiplier input with custom option like barrier */}
                <label>
                  Multiplier
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <select
                      value={multiplierSelect}
                      onChange={e => {
                        const v = e.target.value;
                        setMultiplierSelect(v);

                        if (v === CUSTOM_BARRIER) {
                          setMultiplierInput(multiplierStr || '50');
                          return;
                        }
                      ...
                      {multiplierOptions
                        ...
                        .map(x => (
                          <option key={x} value={x}>
                            {x === CUSTOM_BARRIER ? 'Custom…' : `x${x}`}
                          </option>
                        ))}
                    </select>

                    {customMultiplierMode && (
                      <input
                        type="number"
                        min={1}
                        step={1}
                        ...
                        style={{ width: 120 }}
                        title="Type any multiplier number allowed on the chosen market (e.g. 10, 20, 50, 200)"
                      />
                    )}
                  </div>

                  <div className="muted" style={{ marginTop: 6, fontSize: 12 }}>
                    Tip: multipliers vary by market. If you type an invalid one, the quote will fail and show the reason.
                  </div>
                </label>
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={e => {
                      const v = e.target.value;
                      if (v === '') return setTp('');
                      setTp(clampNum(+v || 0, 0, 1_000_000));
                    }}
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={e => {
                      const v = e.target.value;
                      if (v === '') return setSl('');
                      setSl(clampNum(+v || 0, 0, 1_000_000));
                    }}
                    placeholder="optional"
                  />
                </label>
          ...
          <div className="row">
            ...
            {/* Barrier selector only for vanilla/touch */}
            {mode !== 'multiplier' ? (
              <label>
                {barrierLabel}
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                {mode === 'touch' && barrierChoices.length === 0 && (
                  <div className="muted" style={{ marginTop: 6, fontSize: 12 }}>
                    Auto barrier list will populate when the quote runs (depends on market + duration).
                  </div>
                )}
              ...
              <label>
                ...
                <div className="muted" style={{ marginTop: 8 }}>
                  Multiplier has no duration/barrier. You can close anytime (sell), or TP/SL can auto-close if set.
                </div>
              </label>
            )}
            ...
            <div className="quote-box">
              ...
              <div className="qrow">
                <span className="k">
                  {mode === 'vanilla' ? 'Strike' : mode === 'touch' ? 'Target' : 'Multiplier'}
                </span>
                <span className="v">
                  {mode === 'multiplier'
                    ? Number.isFinite(parsedMultiplier)
                      ? `x${parsedMultiplier}`
                      : '—'
                    : Number.isFinite(proposalStrike)
                      ? proposalStrike.toFixed(2)
                      : '—'}
                </span>
              </div>
            ...
            <button className="buy" onClick={buy} disabled={isBuying || isQuoting}>
              {isBuying
                ...
                : isQuoting
                  ? 'Quoting…'
                  : mode === 'multiplier'
                    ? `Buy Multiplier ${displayDir} (${Number.isFinite(parsedMultiplier) ? 'x' + parsedMultiplier : 'x?'})`
                    : `Buy ${mode === 'vanilla' ? 'Vanilla' : 'T/NT'} ${displayDir}`}
            </button>
          </div>
        </div>
          ...
          <div className="contracts">
            {Object.values(active).map(c => {
              ...
              const isCallish =
                c.mode === 'multiplier'
                  ? c.mult_dir === 'UP'
                  : String(c.dir).includes('CALL') || c.dir === 'TOUCH';
                ...
                <div key={c.contract_id} className="ct-row">
                  <div className="ct-main">
                    ...
                    <div className="ct-sub">
                      <span className="tag">
                        {c.mode === 'vanilla' ? 'VANILLA' : c.mode === 'touch' ? 'T/NT' : 'MULT'}
                      </span>

                      <span className={`tag ${isCallish ? 'call' : 'put'}`}>
                        {c.mode === 'vanilla'
                          ? c.dir
                          : c.mode === 'touch'
                            ? c.dir === 'TOUCH'
                      ...
                      {c.mode === 'multiplier' ? (
                        <>
                          <span className="tag">x{c.multiplier}</span>
                          {c.tp != null && <span className="tag">TP {c.tp}</span>}
                          {c.sl != null && <span className="tag">SL {c.sl}</span>}
                        </>
                      ) : (
                        <>
                          <span className="tag">
                            {c.mode === 'vanilla' ? `offset ${c.barrier_offset}` : `barrier ${c.barrier_offset}`}
                          </span>

                          <span className="tag" title={Number.isFinite(c.profit_line ?? NaN) ? `${c.profit_line}` : ''}>
                            {c.mode === 'vanilla' ? 'strike' : 'target'}{' '}
                            {Number.isFinite(c.profit_line ?? NaN) ? (c.profit_line as number).toFixed(2) : '—'}
                          </span>
                        </>
                      )}
                      ...
                      <span
                        ...
                        title={c.date_expiry ? `Expiry: ${fmtTime(c.date_expiry)}` : 'No fixed expiry'}
                      >
                        exp: {leftTxt}
                      </span>
                    </div>
