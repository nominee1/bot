...
// ⚠️ Auth/Switching now mirrors MultiStrategyBot:
//   - No local transactions subscribe
//   - No local per-contract POC subscribe
//   - Derive currency from current account
//   - Reset local trading state on real account switch
...
// Logger
const L = (...args: any[]) => console.log('[EO-IFRAME]', ...args);
...
const IframeEvenOdd = observer(() => {
  ...
  useEffect(() => {
    if (typeof stakeInput === 'number' && stakeInput > 0) {
      ...
      L('Base stake updated to', baseStakeRef.current);
    }
  }, [stakeInput]);
  ...
  const stopSmartTradesHard = (reason?: 'take_profit' | 'stop_loss') => {
    ...
    setStatus(reason
      ...
      : 'Smart Trade stopped');
    L('STOP HARD', { reason, sessionPL: sessionPLRef.current });
  };
  ...
  // ───── Epoch bump (same pattern as MultiStrategyBot) ─────
  const [apiEpoch, setApiEpoch] = useState(0);
  useEffect(() => {
    const api = api_base.api;
    const conn = api?.connection as any;
    if (!conn) {
      L('No API connection object available yet.');
      return;
    }
    const bump = () => {
      L('WS connection event → bump epoch');
      setApiEpoch(x => x + 1);
    };
  ...
  const [currentCurrency, setCurrentCurrency] = useState<string>(api_base?.account_info?.currency || 'USD');

  // When epoch bumps or api_base updates, pick up fresh account info
  useEffect(() => {
    const loginid = api_base?.account_info?.loginid || null;
    const currency = api_base?.account_info?.currency || 'USD';

    // Detect real account switch → stop + flush locks/state (inputs preserved)
    if (currentLogin && loginid && currentLogin !== loginid) {
      L('ACCOUNT SWITCH DETECTED', { from: currentLogin, to: loginid });
      // mimic MSB behavior: stop trading & clear transient state
      setAutoTradeOn(false);
      inFlightRef.current = false;
      openContractIdRef.current = null;
      ...
      mgCurrentRef.current = baseStakeRef.current;
      mgStepRef.current = 0;
      setStatus('Switched account — trading reset (inputs preserved).');
    }
  ...
  const ensureApiReady = useCallback(async () => {
    ...
    if (!api_base.api || api_base.api.connection.readyState !== OPEN) {
      L('ensureApiReady → re-init/authorize');
      await api_base.init(true); // recreate + authorize + (store-level) re-subs
    }
  }, []);

  // ───── Ticks → digits (analysis) ─────
  ...
  const ensureStakeAndDuration = () => {
    ...
    if (!Number.isFinite(base) || base <= 0) { setStatus('Enter a valid Stake'); L('Invalid stake', stakeInput); return null; }
    if (!Number.isFinite(dur)  || dur < 1)   { setStatus('Enter a valid Duration (ticks)'); L('Invalid duration', ticksInput); return null; }
  ...
  // ───── BUY (guarded + temp row). No local POC/transactions subscribe (MSB parity) ─────
  const placeTrade = async (sig: 'even' | 'odd') => {
    ...
    // derive currency from active account (fallback 'USD')
    ...
    try {
      ...
      // MSB parity: do NOT locally subscribe to proposal_open_contract here.
      // Live updates should arrive via app-level/global subscriptions.
  ...
  const applySettlement = useCallback((_cid: string, net: number) => {
    ...
    if (net >= 0) {
      mgCurrentRef.current = base;
      mgStepRef.current = 0;
      setStatus(`Result: WIN ${net.toFixed(2)} — MG reset to ${base.toFixed(2)}`);
      L('SETTLE WIN', { cid: _cid, net, mg: mgCurrentRef.current });
    } else {
      ...
      L('SETTLE LOSS', { cid: _cid, net, mg: mgCurrentRef.current, step: mgStepRef.current });
    }
    ...
    afterSettleTimerRef.current = setTimeout(() => {
      ...
      if (continueLocked) {
        ...
        if (locked) {
          L('NEXT (locked) → placeTrade', locked);
          placeTrade(locked);
        }
        return;
      }
      ...
      if (sig && hasHeldLongEnough()) {
        L('NEXT (live) → placeTrade', sig);
        placeTrade(sig);
      }
    }, 120);
  }, [continueLocked]);

  // ───── onMessage handler — relies on global subs (MSB parity) ─────
  const handleApiMessage = useCallback(({ data }: any) => {
    if (data?.error) { L('WS ERROR', data.error); return; }
    ...
    if (data?.msg_type === 'proposal_open_contract') {
      ...
      if (finished) {
        ...
        if (!settledContractsRef.current.has(idStr)) {
          ...
          const net = Number(c.profit ?? 0);
          L('PROPOSAL FINISHED → settle', { cid: idStr, net });
          applySettlement(idStr, net);
        }
      }
    }

    // transaction sell events (global subscription)
    if (data?.msg_type === 'transaction' && data.transaction?.action === 'sell') {
      ...
      if (!settledContractsRef.current.has(cid)) {
        ...
        const net = Number(tx.amount) - stake;
        L('TRANSACTION SELL → settle', { cid, net });
        applySettlement(cid, net);
      }
    }
  }, [applySettlement]);

  // Bind onMessage subscriber; re-bind on epoch (MSB pattern)
  useEffect(() => {
    L('Bind onMessage subscriber @ epoch', apiEpoch);
  ...
  useEffect(() => {
    ...
    if (current !== lastSignalRef.current) {
      ...
      L('Signal change', { current, since: signalSinceRef.current });
    }
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      L('Ticks WS open → subscribe history', currentSymbol);
  ...
  const handleToggleRun = () => {
    if (!autoTradeOn) {
      ...
      mgCurrentRef.current = baseStakeRef.current;
      mgStepRef.current = 0;
      L('RUN ON, seed MG from base', baseStakeRef.current);
    }
    setAutoTradeOn(v => !v);
        ...
        <select
          ...
          onChange={(e) => {
            ...
            if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
              L('Ticks WS → switch market', newMarket);
              wsRef.current.send(JSON.stringify({
                ticks_history: newMarket, style: 'ticks', count: 5000, end: 'latest', subscribe: 1,
              }));
            }
        ...
        <div className="threshold">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              const n = Number(v);
              const nv = Math.max(1, Math.min(99, Number.isFinite(n) ? n : 65));
              setThreshold(nv);
              L('Threshold set', nv);
            }}
            min={1}
        ...
        {trades.length === 0 ? (
          ...
          >
            ...
            <div className="position-footer">
              <div className="position-stake">{tr.stake.toFixed(2)} {currentCurrency || 'USD'}</div>
              <div
      ...
      <div className="trade-control-group" style={{ marginTop: 10 }}>
        ...
        <button
          ...
          onClick={() => {
            ...
            L('RESET history, preserve inputs');
          }}
      ...
      <div className="performance-stats">
        <div className="stat-item">
          ...
          <div className={`stat-value ${profitLoss >= 0 ? 'profit' : 'loss'}`}>
            {profitLoss >= 0 ? '+' : '−'}${Math.abs(profitLoss).toFixed(2)} {currentCurrency || 'USD'}
          </div>
        </div>
        <div className="stat-item">
