...
const calculateStreakStats = (history: number[]): StreakStats[] => {
  ...
  ranges.forEach((range, i) => {
    const upperBound = parseInt(range.split('-')[1] || range.split('+')[0]) || Infinity;
...
const AccumulatorPanel: React.FC<{
  symbol: string;
  growthRate: number;
  counter: number;
    ...
    return (
      <div className="accu-panel">
        ...
        {currentRangeStats && (
          ...
          <div className="streak-stats-item">
            <span className="streak-stats-label">Continued</span>
            <span className="streak-stats-value continued">
              {currentRangeStats.continued} ({currentRangeStats.continuationRate.toFixed(1)}%)
            </span>
          </div>
          <div className="streak-stats-item">
